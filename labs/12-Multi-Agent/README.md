...
In this lab, we apply this idea to software development. Instead of asking one model to generate code in a single step, we create a small **multi-agent software team** using **Google Agent Development Kit (ADK)**. The team contains four specialized agents:

The team contains four specialized agents:
...
4. **Reviser Agent**: responsible for improving the code using the design suggestions and review comments.
...
- Build a programmer-designer-reviewer-reviser agent team
...
- **Planning**: deciding how to structure the code, design suggestions, review, or revision;
- **Action**: producing code or feedback;
- **Reflection**: using review to improve previous outputs.

The four main agents in this lab have different responsibilities:
...
In this lab, four specialized LLM agents contribute to the workflow:

- the **Programmer Agent** produces the first working implementation;
- the **Game Designer Agent** proposes gameplay and user-experience improvements;
- the **Reviewer Agent** critiques the code and identifies correctness, maintainability, or design issues;
- the **Reviser Agent** improves the code using the design suggestions and review comments.

The workflow is then organized by ADK orchestration components:

- the **SequentialAgent** defines the overall order of execution;
- the **LoopAgent** repeats the improvement sequence up to three times.
...
6. Add a LoopAgent orchestrator that repeats the design-review-revision cycle.
...
This is important because it allows us to observe what each new role adds to the workflow. A single programmer agent can generate a first version of the Snake game. A reviewer agent can identify weaknesses. A reviser agent can use the review to improve the code. A game designer agent can suggest simple gameplay and user-experience improvements.

The objective is to compare the behavior of the system after each step.

For pedagogical reasons, in this lab you will orchestrate the different agents in a sequential order. The objective is to demonstrate how an agentic pipeline can improve your final results. However, it is entirely possible to define an orchestrator agent responsible for selecting which specialized agent to call depending on the task. For example, in the case of our game, depending on the problem identified, a more advanced orchestrator could decide whether to call the reviewer, the reviser, or the game designer again.

Instead of starting directly with the final orchestration, we first create a simple agent that generates a Snake game. Then, step by step, we add new roles: a reviewer, a reviser, a game designer, and finally a loop orchestrator.
...
At this stage, there is no review, no design feedback, no revision step, and no iteration.

Before to begin, install pygame:
...
Replace your previous `agent.py` with the following code:
...
This shows that agents do not all need to be programmers. Some agents can contribute design, critique, revision, or planning.
...
A multi-agent system can improve a result by introducing specialization, design feedback, review, and revision. However, it can also introduce new problems: longer outputs, unnecessary complexity, inconsistent agent behavior, or false confidence.

The purpose of this lab is therefore not to assume that more agents always produce better results. The purpose is to examine whether a structured workflow helps produce code that is easier to understand, review, run, and improve.
...
- - code being difficult to understand, run, or modify.
